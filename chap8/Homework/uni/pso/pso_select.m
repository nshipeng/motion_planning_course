...
vmax = 1; vmin = -1;
theta_max = 0.9 * pi; theta_min = -0.9 * pi;
c1 =2; c2 =2; c3 = 2; c4 = 2;
for j=1:batch
    for i=1:N
        ...
        if (j~=1)
            ...
            P(i,3) = P(i,3) * w + c1* rand() * (P(i,5) - P(i,1)) + c2 * rand() * (global_best(1) - P(i,1)); 
            P(i,4) = P(i,4) * w + c3* rand() * (P(i,6) - P(i,2)) + c4 * rand() * (global_best(2) - P(i,2)); 
            P(i,1) = P(i,1) +P(i,3);   
            P(i,2) = P(i,2) + P(i,4); 
